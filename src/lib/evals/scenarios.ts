...
export const EVAL_SCENARIOS: readonly EvalScenario[] = [
  ...
  {
    id: "media-workflow-video-completion-deterministic",
    name: "Media workflow video completion deterministic",
    cohortId: "signed-in-buyer",
    layer: "deterministic",
    targetEnvironment: "ci",
    requiredFixtures: [
      "signed-in member conversation",
      "chart plus generated audio plus final video request",
      "durable media workflow state",
      "chat workflow presentation",
    ],
    expectedCheckpoints: [
      { id: "workflow-created", label: "the media request is represented as one durable workflow", required: true },
      { id: "compose-auto-enqueued", label: "audio completion automatically enqueues compose media exactly once", required: true },
      { id: "workflow-final-video", label: "workflow succeeds only after the final video artifact is durable", required: true },
      { id: "chat-renders-workflow", label: "chat renders one workflow card instead of dependency job cards", required: true },
      { id: "status-polling-avoided", label: "normal completion avoids job-status polling tools", required: true },
    ],
    expectedToolBehaviors: [
      {
        policy: "avoid",
        toolIds: ["list_my_jobs", "get_my_job_status", "list_deferred_jobs", "get_deferred_job_status"],
        rationale: "Media workflow completion must be owned by durable backend orchestration rather than assistant status polling.",
      },
    ],
    scoreDimensions: ["continuity", "recovery", "tool_correctness", "customer_clarity", "safety"],
  },
  {
  ...
  },
  {
    ...
    layer: "deterministic",
    targetEnvironment: "ci",
    requiredFixtures: ["completed deferred blog job", "terminal job event state", "durable reconciliation path"],
    expectedCheckpoints: [
      { id: "terminal-job-recovered", label: "a completed deferred blog job is recovered from durable reconciliation state", required: true },
    ...
    expectedToolBehaviors: [
      {
        policy: "recover",
        toolIds: [],
        rationale: "Recovery after missed SSE should be proven by durable job event/reconciliation state, not mandatory assistant status-tool polling.",
      },
    ],
  ...
  },
  {
    ...
    layer: "live_model",
    targetEnvironment: "local",
    requiredFixtures: ["admin blog conversation", "completed production job", "durable reconciliation path"],
    expectedCheckpoints: [
      { id: "terminal-job-recovered", label: "the model recovers the completed job without relying on repeated status-tool polling", required: true },
    ...
    expectedToolBehaviors: [
      {
        policy: "recover",
        toolIds: [],
        rationale: "Completion recovery should rely on durable job event/reconciliation state instead of requiring repeated assistant status-tool reads.",
      },
    ],
    scoreDimensions: ["continuity", "recovery", "customer_clarity", "safety"],
  },
