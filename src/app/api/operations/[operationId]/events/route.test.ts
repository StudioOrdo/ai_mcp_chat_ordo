...
function repository(): OperationRepository {
  return {
    ...
    listEvents: vi.fn(async () => [{
      ...
      stepId: null,
      sequence: 1,
      type: "operation_created" as const,
...
describe("GET /api/operations/[operationId]/events", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getSessionUserMock.mockResolvedValue({ id: "usr_1", email: "u@example.com", name: "User", roles: ["AUTHENTICATED"] });
  });

  it("returns operation events through the repository", async () => {
    const repo = repository();
    getOperationRepositoryMock.mockReturnValue(repo);

    const response = await GET(new NextRequest("http://localhost/api/operations/op_1/events?afterSequence=0&limit=10"), {
      params: Promise.resolve({ operationId: "op_1" }),
    });
    const payload = await response.json();

    expect(response.status).toBe(200);
    expect(payload.events[0].id).toBe("evt_1");
    expect(repo.listEvents).toHaveBeenCalledWith("op_1", { afterSequence: 0, limit: 10 });
  });
});
