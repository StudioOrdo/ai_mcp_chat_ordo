...
describe("useComposeMediaMaterialization", () => {
  ...
  it("materializes chart clips from transcript payloads when governed storage fetch fails", async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response("governed-unavailable", { status: 500 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({
        attachments: [{ assetId: "uf_chart_png_1", mimeType: "image/png", assetKind: "image", source: "derived", retentionClass: "conversation" }],
    ...
    expect(materialized.visualClips).toEqual([{ assetId: "uf_chart_png_1", kind: "image", sourceAssetId: "uf_chart_1" }]);
  });
